# intake-bug-20260628-bug0028 — BUG-0028 Category spending trend shows only one month (regression of BUG-0025)

**From:** PO **To:** Tech Lead **Bug:** BUG-0028 **Run:** `intake-bug-20260628-bug0028`
**Date:** 2026-06-28 **Next phase:** `/discovery` (role: po)
**Intake evidence:** inline (see § below)

## Bug summary

Operator report (German, 2026-06-28): Category spending trend on `/forecast` and `/wealth` shows **only one month** with expenses (~€450 for 2026-05 Stromkosten) while all other months (2025-07 through 2026-04, 2026-06) show **€0** — despite operator having recurring monthly Stromkosten transactions across multiple months. Issue reproduces across **multiple categories**, not just Stromkosten.

**Regression of BUG-0025 (BW).** BUG-0025 code fix merged to `main` (Q0034 verify-work: `PASS-WITH-PREREQUISITES`) but the **omniflow backend rebuild + deploy was never completed**. Live container runs pre-fix scheduled-sync path (`watermark − 7d` overlap window), so backdated transactions outside the 7-day window are silently skipped. Backend SQL + frontend code are correct; the mirror is stale.

## Acceptance rows (canonical: `docs/product/acceptance.md` BUG-0028)

- **(CE)** After rebuild + manual Full sync, `/forecast` and `/wealth` Category spending trend for any recurring-expense category shows **monthly bars for each month** with data — not single-month-with-€450 pattern.
- **(CF)** Manual Full sync fetches ≥365 days of Firefly txs (`MANUAL_LOOKBACK_DAYS`); categories with multi-month history render multi-month bars.
- **(CG)** Categories with genuinely single-month history still render faithfully (one bar + €0) — mirror fidelity preserved.
- **(CH)** `GET /api/v1/categories/expense-series?category_id=N` returns non-zero bar per month with Firefly data for N.

## Sub-defects confirmed

| AC | Verdict | Key evidence |
|----|---------|--------------|
| **BW (regression)** | **CONFIRMED** (live; code PASS on `main`) | Screenshot 2026-06-28: only 2026-05 bar ~€445 for Stromkosten (146) |
| **BX** | **OPEN** (operator needs to re-run manual sync post-deploy) | Not tested — blocked on rebuild |
| **BY** | **PASS** (code; 365d lookback on "Sync now") | `backend/src/firefly/mod.rs` L376-391 |

## Root-cause chain (code + live)

| Layer | Finding |
|-------|---------|
| SQL (`repository.rs` L269-342) | `generate_series` month-spine + LEFT JOIN — **correct**; unit tests pass multi-month |
| Frontend (`CategoryTrendChart.tsx`) | Renders API output as-is — **correct** |
| Service (`service.rs` L189-250) | `expense_series_window` computes 12-month window correctly |
| Firefly sync (`firefly/mod.rs` L376-391) | Scheduled: `watermark − overlap_days` (default 7d); Manual: 365d lookback (**Q0034 fix on disk, not on live image**) |
| Mirror (live) | Category 146 (Stromkosten): single-month data — matches scheduled 7d window ingest only |
| Deploy (Q0035 = BUG-0027 PAT) | No BUG-0025 fix included; omniflow image predates Q0034 |

## UX / design recommendation (PO)

N/A — no UX change; data-freshness / deployment gap only.

## Architecture gates (mandatory decisions)

| Gate | Question | TL default |
|------|----------|------------|
| **GATE-DEPLOY-1** | Rebuild + redeploy on omniflow | Required — no code change needed to close |
| **GATE-SYNC-1** | Operator re-runs "Sync now" post-deploy | Required to repopulate mirror |
| **GATE-RESET-1** | Data older than 365 days | Optional — `DELETE FROM sync_cursors WHERE entity_type='transactions'` first, then manual sync (runbook L1984-2001) |
| **GATE-VERIFY-1** | Post-remediation smoke | `GET /api/v1/categories/expense-series?category_id=146` multi-month bars |
| **GATE-DEC-1** | New DEC? | No — BUG-0025 (R-0097 / Q0034) already canonicalizes the fix |

## Scope / risks

| Area | Note |
|------|------|
| Code | Zero — fix on `main`; rebuild required only |
| Operator ops | Rebuild + manual sync + verify |
| Data completeness | Transactions older than 365 days need cursor reset |
| Regression | Existing BUG-0025 acceptance rows unchanged; CE/CF/CG/CH new for BUG-0028 |
| OIDC deploy | Smoke per acceptance |

## Decomposition

**Single bug** — undeployed fix + stale mirror. Reopening BUG-0025 rejected (DONE on disk; new live evidence warrants new bug-id). No new code.

## Intake evidence (BUG-0028)

- `intake_run_id`: `intake-bug-20260628-bug0028`
...
- `assumptions_confirmed`: BUG-0025 fix present on `main` (code PASS Q0034) but not deployed to omniflow; operator confirmed multiple months of Stromkosten txs in Firefly; manual Full sync 365d lookback is the minimal post-deploy remediation. No new code change required to close BUG-0028 — deployment + sync only.

## Related work

**BUG-0025** (BW/BX/BY — original root cause, code-fixed); **BUG-0026** related category-trend surface; **Q0034** verify-work `PASS-WITH-PREREQUISITES` on `main`; **Q0035** deploy = BUG-0027 PAT only (no BUG-0025); **R-0097** (§ BUG-0025 investigation).

**Recommended next phase**

`/discovery` (or direct ops) — rebuild + redeploy omniflow, run manual Full sync, run verify probes CE/CF/CH per acceptance.
