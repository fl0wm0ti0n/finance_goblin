...
- [x] **BUG-0022** — **(BM)** With two or more plans on `/planning`, selecting a **non-active** plan in the dropdown enables **Delete plan**; confirmation removes the plan and refreshes the list—not a permanently disabled delete control when a global active plan exists. **(BN)** Deleting the globally **active** plan remains blocked in UI (disabled + tooltip) and via `DELETE /api/v1/plans/:id` (**409** per **DEC-0082**). OIDC-enabled deploy regression checks pass *(code + vitest 17/17 + BN-API live 409 PASS; BM/BN browser operator-deferred pending **FRONTEND_DEPLOY**)*.

- [x] **BUG-0023** — **(BO)** With Bitunix connected and exchange sync success, Wealth **Crypto** tab Bitunix card and `GET /api/v1/wealth` `crypto.subtotal_eur` reflect operator portfolio value (~**€2000** order of magnitude per operator Bitunix app)—not **€0** / **€-0,00** with **11** open linear positions. **(BP)** Holdings table **Value EUR** column shows EUR equivalent at valuation time for linear positions when prices available—not all **—** while **Native qty** and **Unrealized PnL** are populated. **(BQ)** PnL summary **Total return %** populated when baseline exists after exchange sync + PnL recompute—not **—** while unrealized EUR is non-zero. OIDC-enabled deploy regression checks pass *(code + integration 4/4 PASS; BO/BP/BQ live API/UI operator-deferred pending **BACKEND_DEPLOY → EXCHANGE_SYNC → PNL_RECOMPUTE**)*.

- [x] **BUG-0024** — **(BR)** With **two or more** plans on `/planning`, selecting a **non-active** plan in the dropdown enables **Delete plan** and successful confirmation removes the plan—not permanently disabled after **Q0031** deploy. **(BS)** With **only one** globally active plan, **Delete plan** remains disabled per **DEC-0082** but UI shows **clear explanation** (tooltip or inline copy) describing how to delete (e.g. create another plan, set it active, then delete the original)—not silent gray button with no guidance. OIDC-enabled deploy regression checks pass *(code + vitest 31/31 + BR browser/API PASS; BS inline hint operator-deferred pending **FRONTEND_DEPLOY**)*.

- [x] **BUG-0025** — **(BW)** After operator adds **multiple** Firefly transactions to category **Wohnen - Stromkosten** across **multiple months** and runs **manual Full Firefly sync**, **`/forecast` Category spending trend** (and `GET /api/v1/categories/expense-series`) shows outflow bars for **each month** with data—not **only 2026-05** with all prior months at **€0** while Firefly holds Stromkosten rows in those months. **(BX)** **Backdated** or **bulk-imported** Firefly transactions ingest on next appropriate sync **or** Sync Status / operator docs explain the **DEC-0002** overlap limitation and remediation (cursor reset / full re-sync)—not silent omission with misleading success. **(BY)** Manual **Sync now** on Sync Status triggers **Full** Firefly ingest; sync history distinguishes Firefly full runs (`manual`, `scheduled`) from exchange-only (`scheduled_exchanges`, `manual_exchanges`). OIDC-enabled deploy regression checks pass *(code + integration 3/3 + npm 31/31 PASS; BX-DOC + BY-HIST PASS; BW/BX/BY live operator-deferred pending **BACKEND_REBUILD** + **FRONTEND_DEPLOY**)*.

- [x] **BUG-0026** — **(BZ)** On `/forecast` **Monthly** tab, summary **Income** card is **consistent** with the monthly bar chart for the **same labeled reference month**—not **0.00** on the card while the chart shows non-zero Income bars (~€3000) for visible forecast months without explaining the mismatch. **(CA)** Summary cards show **which month** they represent (or use current/next full-month semantics documented in UI)—not unlabeled values from `series[0]` when that month differs from operator-expected forecast view. OIDC-enabled deploy regression checks pass *(code + vitest 7/7 PASS; BZ-API live series[1] 3266.16 PASS; BZ/CA browser operator-deferred pending **FRONTEND_DEPLOY**)*.

- [x] **BUG-0027** — **(CB)** After regenerating Firefly PAT in Firefly profile → API tokens and updating `FIREFLY_PERSONAL_ACCESS_TOKEN` in `/workdir/financegoblin/.env` (or operator `.env`) + recreating container, `GET /api/v1/sync/status` shows `state: completed` after next scheduled run; manual "Sync now" returns `status: completed` with non-zero entity counts; no `error_message` containing `401` or `Unauthorized`. Live Firefly mirror reflects current data. — **DONE** verified 2026-06-25 post-v1 operator smoke; sync status returned `state: success` after scheduled run. **(CC)** App surfaces clear user-facing diagnosis on `/sync` when PAT auth fails (distinguishing "PAT expired/invalid" from "PAT missing" and "Firefly unreachable") — operator knows to regenerate. — **DONE** (code verified). **(CD)** Regression: subsequent scheduled syncs succeed across ≥3 cycles (no silent 401 recurrence) after PAT regen. OIDC-enabled and omniflow external deploy regression checks pass. — **DONE** operator confirmed sync operational 2026-06-25 19:03Z.

- [ ] **BUG-0028** — **(CE)** After rebuilding omniflow container with BUG-0025 fix (`main` branch) and running **Sync now** (manual Full sync), `/forecast` and `/wealth` **Category spending trend** for any expense category with recurring transactions shows **monthly bars for each month** in the 12-month window where Firefly holds categorized data — not only-a-single-month-with-€450 pattern for Stromkosten or similar recurring categories. **(CF)** Manual Full sync fetches ≥365 days of Firefly transactions (per `MANUAL_LOOKBACK_DAYS`); categories with multi-month history render multi-month bars. **(CG)** Categories with genuinely single-month history still render with one bar + other months €0 (mirror fidelity; not a false positive). **(CH)** `GET /api/v1/categories/expense-series?category_id=N` returns a non-zero bar for each month with Firefly transactions for category N; response shape matches DEC-0007 expense-series contract. OIDC-enabled deploy regression checks pass.
...
- [x] **AC-1 Transaction search:** `/subscriptions` Discover (or equivalent) lists individual mirror **expense transactions** with pagination/cap — not limited to pre-grouped `detect_recurrence_groups` candidates only *(code + integration PASS; live tx-search browser operator-deferred pending **BACKEND_FRONTEND_DEPLOY**)*
- [x] **AC-2 Rich filters:** Operator can filter by **account**, **payee/description** text, **category**, **Geldbereich** (`account_role`), and **date range**; optional amount band and recurring/pattern-hint mode documented in UI *(code + integration PASS; live filter UI operator-deferred pending **BACKEND_FRONTEND_DEPLOY**)*
- [x] **AC-3 Pattern hint:** When filtered transactions form a recurring expense pattern (including below auto-detection emit threshold), UI surfaces interval/amount hint or selectable transaction group for review *(hint test + badge UI PASS; account 114 live fixture operator-deferred pending **BACKEND_FRONTEND_DEPLOY**)*
- [x] **AC-4 Manual activate:** Operator confirms selected transaction group as **subscription** or **standing order** without pending-only auto-detection path; respects **DEC-0085** merge and rejection rules *(preview-group + multi-select UI PASS; live confirm operator-deferred pending **BACKEND_FRONTEND_DEPLOY**)*
- [x] **AC-5 Regression:** US-0020 tags, majority category, and US-0003/US-0008 detection + alert dedup unchanged *(integration `reg_discover_candidate_pass_unchanged_ac5` PASS; patterns tab preserved)*
- [x] **AC-6 OIDC:** External profile smoke pass on Discover transaction search and confirm flow *(health + discover API 200; OIDC browser operator-deferred pending **BACKEND_FRONTEND_DEPLOY**)*

## US-0022 — Deploy version stamp & stale-frontend detection

- [x] **AC-1 Version stamp:** App shell shows a **subtle** version/build indicator (footer or sidebar corner) that does not dominate primary UX; default view is minimal (e.g. short label or icon)
- [x] **AC-2 Hover details:** Hover or focus reveals **release tag**, **build id** (git short sha or equivalent), and **build timestamp** (UTC) — sufficient to distinguish pre/post deploy (e.g. Q0030 vs pre-2026-06-09 image)
- [x] **AC-3 Backend metadata:** `GET /health` extended or `GET /api/v1/meta/build-info` returns authoritative **build id** and **release tag** matching Docker/build pipeline — no secrets
- [x] **AC-4 SPA embed:** Frontend bundle embeds the same **build id** at compile time (`VITE_BUILD_ID` or equivalent from Docker build args)
- [x] **AC-5 Stale detection:** When SPA-embedded build id ≠ backend authoritative id, UI shows a **non-blocking** stale warning with reload affordance; no false positive when ids match after deploy *(code PASS; live browser stale-detection operator-deferred pending **BACKEND_FRONTEND_DEPLOY**)*
- [x] **AC-6 Regression:** `/health` liveness unchanged; OIDC external profile smoke pass; metadata responses contain no env secrets *(code+test PASS; OIDC smoke operator-deferred pending **BACKEND_FRONTEND_DEPLOY**)*

## US-0023 — Multi-category trend chart with cumulative toggle

- [ ] **AC-1 Multi-line chart:** `/forecast` **Monthly** tab and `/wealth` **Overview** render **Category spending trend** as a colored **line chart** with one line per category (ECharts); each line has a distinct color and legend label; no hard limit on simultaneous categories
- [ ] **AC-2 Multi-select filter:** Shared multi-select (checkbox list or chips) lets operator toggle categories in/out of the chart; "select all" / "deselect all" affordance present; default view = top 5 categories by total outflow in window, or last-saved selection if present
- [ ] **AC-3 Cumulative toggle:** Chart supports **two modes** — (a) per-month outflow (default) and (b) cumulative / running sum across the visible window, toggled via a single button/chip; cumulative label updates to indicate active mode
- [ ] **AC-4 Persistence:** Multi-select state + cumulative toggle saved to **localStorage** (and/or URL params); page reload restores last session; first visit uses default top-5
- [ ] **AC-5 Zero-outflow rendering:** Months where a category has €0 spend display as a flat line at the x-axis (not line break/gap); line remains continuous across window
- [ ] **AC-6 Regression:** US-0018 single-category MVP behavior preserved for operators who select exactly one category; `GET /api/v1/categories/expense-series` contract unchanged; OIDC-enabled deploy smoke pass
