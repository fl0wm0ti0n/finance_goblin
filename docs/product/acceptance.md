...
- [ ] **BUG-0027** — **(CB)** After regenerating Firefly PAT in Firefly profile → API tokens and updating `FIREFLY_PERSONAL_ACCESS_TOKEN` in `/workdir/financegoblin/.env` (or operator `.env`) + recreating container, `GET /api/v1/sync/status` shows `state: completed` after next scheduled run; manual "Sync now" returns `status: completed` with non-zero entity counts; no `error_message` containing `401` or `Unauthorized`. Live Firefly mirror reflects current data. **(CC)** App surfaces clear user-facing diagnosis on `/sync` when PAT auth fails (distinguishing "PAT expired/invalid" from "PAT missing" and "Firefly unreachable") — operator knows to regenerate. **(CD)** Regression: subsequent scheduled syncs succeed across ≥3 cycles (no silent 401 recurrence) after PAT regen. OIDC-enabled and omniflow external deploy regression checks pass.
